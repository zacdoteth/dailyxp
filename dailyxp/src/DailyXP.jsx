...
const getPastWeekIds = (n) => {
  const weeks = [];
  const today = getToday();
  let wid = getWeekId(today);
  for (let i = 0; i < n; i++) {
    weeks.unshift(wid);
    const d = new Date(wid + "T12:00:00");
    d.setDate(d.getDate() - 7);
    wid = d.toISOString().split("T")[0];
  }
  return weeks;
};
...
export default function DailyXP() {
  ...
  useEffect(() => {
    document.fonts.ready.then(() => {
      if (mnt.current) setFontsReady(true);
    });
  }, []);

  useEffect(() => {
    if (!intro || !fontsReady) return;
    const t0 = setTimeout(() => setIntroPhase(1), 50);
    const t1 = setTimeout(() => setIntroPhase(2), 700);
    const t2 = setTimeout(() => setIntroPhase(3), 1900);
    const t3 = setTimeout(() => setIntro(false), 2700);
  ...
  if (loading || !data || !fontsReady) return (
    <div style={{ background: P.void, height: "100dvh" }} />
  );

  if (intro) return (
    <div style={{ background: P.void, height: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
      {/* Aurora glow behind logo */}
      <div style={{ position: "absolute", width: 300, height: 300, borderRadius: "50%", background: `radial-gradient(circle, ${P.gold}18 0%, transparent 70%)`, filter: "blur(60px)", opacity: introPhase >= 3 ? 0 : 0.6, transition: "opacity 0.8s ease" }} />
      {/* Logo */}
      <div style={{ opacity: introPhase >= 1 && introPhase < 3 ? 1 : 0, transform: introPhase >= 1 ? "scale(1) translateY(0)" : "scale(0.8) translateY(8px)", transition: "all 0.8s cubic-bezier(0.16, 1, 0.3, 1)", display: "flex", alignItems: "baseline", gap: 0 }}>
        <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 52, fontWeight: 800, color: P.text1, letterSpacing: -2 }}>DAILY</span>
        <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 52, fontWeight: 800, color: P.gold, letterSpacing: -2 }}>XP</span>
      </div>
      {/* Tagline */}
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 440, margin: "0 auto", padding: "env(safe-area-inset-top, 0px) 20px calc(env(safe-area-inset-bottom, 0px) + 48px)" }}>
        ...
        {view === "week" && (
          <div style={{ animation: "fadeUp 0.35s ease both" }}>
            ...
            {(() => {
              const weeks = getPastWeekIds(8);
              const scores = weeks.map(w => calcWeekScore(data.days, data.quests, w));
              const maxS = Math.max(...scores, 1);
              const hasData = scores.some(s => s > 0);
              if (!hasData) return null;
              const W = 360, H = 120, padX = 10, padY = 14;
              const chartW = W - padX * 2, chartH = H - padY * 2;
              const points = scores.map((s, i) => ({
                x: padX + (i / (scores.length - 1)) * chartW,
                y: padY + chartH - (s / maxS) * chartH,
                s, wid: weeks[i],
              }));
              ...
              return (
                <div style={{ background: P.surface, border: `1px solid ${P.border}`, borderRadius: 16, padding: "18px 16px", marginBottom: 20 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
                    <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 11, fontWeight: 500, letterSpacing: 2, color: P.text3, textTransform: "uppercase" }}>Weekly History</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      <TrendingUp size={11} color={P.text3} strokeWidth={1.5} />
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: P.text3 }}>8 weeks</span>
                    </div>
                  </div>
                  <svg viewBox={`0 0 ${W} ${H + 20}`} width="100%" style={{ overflow: "visible" }}>
                    {/* Grid lines */}
                    {[0, 0.25, 0.5, 0.75, 1].map(pct => (
                      <line key={pct} x1={padX} x2={W - padX} y1={padY + chartH - pct * chartH} y2={padY + chartH - pct * chartH} stroke={P.text4} strokeWidth="0.5" />
                    ))}
                    {/* Gradient fill */}
                    <defs>
                    ...
                    {points.map((p, i) => (
                      <g key={i}>
                        <circle cx={p.x} cy={p.y} r={weeks[i] === currentWeek ? 4 : 2.5} fill={weeks[i] === currentWeek ? P.gold : P.text2} stroke={weeks[i] === currentWeek ? P.goldBright : "none"} strokeWidth="1.5" />
                        {p.s > 0 && <text x={p.x} y={p.y - 8} textAnchor="middle" fill={weeks[i] === currentWeek ? P.gold : P.text3} fontSize="9" fontFamily="'JetBrains Mono',monospace" fontWeight={weeks[i] === currentWeek ? 700 : 400}>{p.s}</text>}
                        <text x={p.x} y={H + 14} textAnchor="middle" fill={weeks[i] === currentWeek ? P.text1 : P.text3} fontSize="8" fontFamily="'Outfit',sans-serif">{getWeekLabel(p.wid)}</text>
                      </g>
                    ))}
                  </svg>
                </div>
        ...
        {view === "config" && editQ && (
          <div style={{ animation: "fadeUp 0.35s ease both" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
              <button onClick={() => {
                let totalXP = 0;
                Object.values(data.days).forEach(d => { editQ.forEach(q => { if (d[q.id]) totalXP += q.points; }); });
                const ws = calcWeekScore(data.days, editQ, getWeekId(getToday()));
                save({ ...data, quests: editQ, totalXP, weekHigh: Math.max(data.weekHigh, ws), allTimeHigh: Math.max(data.allTimeHigh, ws) });
                setEditQ(null); setView("today");
              }} style={{ flex: 2, background: `linear-gradient(135deg,rgba(61,214,140,0.1),rgba(56,189,248,0.08))`, border: `1px solid rgba(61,214,140,0.2)`, borderRadius: 14, padding: 14, color: P.emerald, fontFamily: "'Outfit',sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Save config</button>
